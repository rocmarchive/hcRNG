...
hcrngStatus hcrngMrg32k3aDeviceRandomIntegerArray_single(hc::accelerator_view accl_view, size_t streamCount, hcrngMrg32k3aStream *streams,
        size_t numberCount, int a, int b, int *outBuffer, int streamlength, size_t streams_per_thread)
{
        ...
        hc::parallel_for_each(accl_view, t_ext, [ = ] (hc::tiled_index<1> tidx) [[hc, cpu]] {
           ...
           for(int i =0; i < (numberCount-1)/streamCount+1; i++) {
              for (int j = 0; j < streams_per_thread; j++)
               if ((streams_per_thread * (i * (streamCount/streams_per_thread) + gid) + j) < numberCount)
                outBuffer[streams_per_thread * (i * (streamCount/streams_per_thread) + gid) + j] = hcrngMrg32k3aRandomInteger(&streams[streams_per_thread * gid + j], a, b);
              }
           }
        }).wait();
        return status;
...
hcrngStatus hcrngMrg32k3aDeviceRandomIntegerArray_double(hc::accelerator_view accl_view, size_t streamCount, hcrngMrg32k3aStream *streams,
        size_t numberCount, int a, int b, int *outBuffer, int streamlength, size_t streams_per_thread)
{
        if (streamCount < 1)
                return HCRNG_INVALID_VALUE;
...
hcrngStatus hcrngMrg32k3aDeviceRandomUnsignedIntegerArray_single(hc::accelerator_view accl_view, size_t streamCount, hcrngMrg32k3aStream *streams,
        size_t numberCount, unsigned int a, unsigned int b, unsigned int *outBuffer, int streamlength, size_t streams_per_thread)
{
        ...
        hc::parallel_for_each(accl_view, t_ext, [ = ] (hc::tiled_index<1> tidx) [[hc, cpu]] {
           ...
           for(int i =0; i < (numberCount-1)/streamCount+1; i++) {
              for (int j = 0; j < streams_per_thread; j++)
               if ((streams_per_thread * (i * (streamCount/streams_per_thread) + gid) + j) < numberCount)
                outBuffer[streams_per_thread * (i * (streamCount/streams_per_thread) + gid) + j] = hcrngMrg32k3aRandomUnsignedInteger(&streams[streams_per_thread * gid + j], a, b);
              }
           }
        }).wait();
        return status;
...
hcrngStatus hcrngMrg32k3aDeviceRandomUnsignedIntegerArray_double(hc::accelerator_view accl_view, size_t streamCount, hcrngMrg32k3aStream *streams,
        size_t numberCount, unsigned int a, unsigned int b, unsigned int *outBuffer, int streamlength, size_t streams_per_thread)
{
        if (streamCount < 1)
                return HCRNG_INVALID_VALUE;
