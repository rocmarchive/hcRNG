...
Create buffer to transfer streams to the device. acc[1] denotes that you have allocated pointer in device (1st GPU). To create a pointer in device, the user has to enumerate the list of accelerators and select the desired accelerator.
...
The user has to call the kernel depending on the type of output buffer. Kernels of type float and double has suffixes "_single" and "_double" respectively. 
