...
TEST(mrg32k3aDouble_test_normal, Functional_check_mrg32k3aDouble_normal)
{
        ...
        status = hcrngMrg32k3aDeviceRandomNArray_double(streamCount, streams_buffer, numberCount, 0.0, 1.0, outBufferDevice);
        EXPECT_EQ(status, 0);
        ...
        status = hcrngMrg32k3aDeviceRandomNArray_double(streamCount, streams_buffer, numberCount, 0.0, 1.0, outBufferDevice_substream, stream_length, streams_per_thread);
        EXPECT_EQ(status, 0);
