...
TEST(lfsr113Single_test_normal, Functional_check_lfsr113Single_normal)
{
        ...
        status = hcrngLfsr113DeviceRandomNArray_single(streamCount, streams_buffer, numberCount, 0.0, 1.0, outBufferDevice);
        EXPECT_EQ(status, 0);
        ...
        status = hcrngLfsr113DeviceRandomNArray_single(streamCount, streams_buffer, numberCount, 0.0, 1.0, outBufferDevice_substream, stream_length, streams_per_thread);
        EXPECT_EQ(status, 0);
